...
class WsabiMGP(WarpedGP):
    """An approximate model for a GP using a square-root warping of the output space, using moment-matching.
    ...
    def posterior_mean_and_variance(self, x: ndarray) -> Tuple[ndarray, ndarray]:
        ...
        mean = self._alpha + 0.5 * (gp_mean ** 2 + gp_variance)
        variance = 0.5 * gp_variance ** 2 + gp_variance * gp_mean ** 2
    ...
    def posterior_variance_jacobian(self, x: ndarray) -> ndarray:
        ...
        # now we have 2 terms, one is equal to that of WSABI-L
        # but we also need the jacobian of 0.5 * gp_variance ** 2
        # which is gp_variance * gp_variance_jacobian
        
        bigterm = jacobian_of_f_squared_times_g(
            f=gp_mean, f_jacobian=gp_mean_jacobian,
            g=gp_variance, g_jacobian=gp_variance_jacobian)
        smallterm = gp_variance.reshape(-1,1) * gp_variance_jacobian
        
        """
        print("big term has shape:")
        print(bigterm.shape)
        print("small term has shape:")
        print(smallterm.shape)
        """
        
        return bigterm + smallterm
...
class LogMGP(WarpedGP):
    """An approximate model for a GP using a log of the output space, using moment-matching.
    ...
    def __init__(self, gp: Union[GP, GPy.core.GP]) -> None:
        # here, alpha takes the role of shifting
        # so that in the warped space, the maximum observation is 0.
        # we achieve this by shifting in the unwarped space so that the maximum observation is 1.
        #self._alpha = 0  # alpha must be initialised first since it may be needed by super().__init__
        super().__init__(gp)
        #self._alpha = 1.-np.max(*self._observed_Y)
    ...
    def posterior_mean_and_variance(self, x: ndarray) -> Tuple[ndarray, ndarray]:
        ...
        mean = np.exp(gp_mean + 0.5 * gp_variance)
        variance = mean ** 2 * (np.exp(gp_variance) - 1)
    ...
    def posterior_variance_jacobian(self, x: ndarray) -> ndarray:
        ...
        bigterm = jacobian_of_f_squared_times_g(
            f=gp_mean, f_jacobian=gp_mean_jacobian,
            g=(np.exp(gp_variance)-1), g_jacobian=np.exp(gp_variance_jacobian)*gp_variance_jacobian)
        return bigterm
    ...
    def _warp(self, y: ndarray) -> ndarray:
        #print("warped data:")
        #print(np.log(y + self._alpha))
        if np.min(y) < 1.:
            print("observation < 1 found. be careful!")
        return np.log(y)
